...
import sys
import numpy as np
from singlemultileavesimulation import SingleMultileaveSimulation
...
from utils.averagemultileaveoutput import MultileaveOutputAverager


class MultileaveSimulation(object):
    ...
    def __init__(self, simulation_arguments):
        ...
        self.folds_in_mem = 0
        if simulation_arguments.max_folds == None:
            self.max_folds = 999
        else:
            self.max_folds = simulation_arguments.max_folds

        self.output_averager = MultileaveOutputAverager(simulation_arguments)
    ...
    def load_processes(self, datasets, ranker_tuples):
        ...
        for dataset in datasets:
            ...
            while self.folds_in_mem >= min(dataset.max_folds, self.max_folds):
                self.wait_for_output()

        # self.all_launched[dataset] = True

    def load_datafold_processes(self, datafold, ranker_tuples):
        while self.folds_in_mem >= min(datafold.max_folds, self.max_folds):
            self.wait_for_output()
        ...
        for arg_str, run_name, r_class, r_args, r_kargs in ranker_tuples:
            ...
            for c_m in self.click_models[datafold.click_model_type]:
                sim = SingleMultileaveSimulation(self.sim_args, self.output_queue, c_m, datafold)
    ...
    def update_active(self):
        ...
        self.max_folds = min([self.max_folds] + [datafold.max_folds for datafold in alive_folds])
